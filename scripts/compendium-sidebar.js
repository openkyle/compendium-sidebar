...
function buildTree(pack, entries, tab) {
  ...
  const appendLevel = (target, parentId) => {
    for (const folder of (byParent.get(parentId) ?? []).sort(sort)) {
      ...
      li.innerHTML = `<header class="folder-header"><i class="fas ${expanded ? "fa-folder-open" : "fa-folder"} folder-icon" aria-hidden="true"></i><span class="folder-name">${escapeHtml(folder.name)}</span></header><ol class="subdirectory"></ol>`;
